...
def trend(product: str) -> dict:
    ...
    return {"title": {"text": f"{product} 单位成本趋势（2026）"},
...
def waterfall(product: str, month: str) -> dict:
    ...
    return {"title": {"text": f"{month} {product} 单位成本变动分解"},
...
def structure(product: str, month: str) -> dict:
    ...
    return {"title": {"text": f"{month} {product} 成本结构"},
...
def heatmap() -> dict:
    ...
    return {"title": {"text": "要素×产品×月份 热力图（元/盒）"},
            "xAxis": {"type": "category",
                      "data": [f"{p}-{m}月" for p in PRODUCTS for m in range(1, 7)]},
            "yAxis": {"type": "category", "data": ["材料", "人工", "制费"]},
